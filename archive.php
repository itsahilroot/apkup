      ...
      <aside class="w-full lg:w-1/4 flex flex-col gap-6">
        ...
        <div class="glass-card p-6 rounded-[28px] border border-slate-200/50 dark:border-white/5 space-y-4">
          <h3 class="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">
            <?php esc_html_e('Categorías', 'apktemplates'); ?>
          </h3>
          <ul class="flex flex-col gap-2.5 text-xs font-semibold text-slate-600 dark:text-slate-400 list-none pl-0">
            ...
            foreach ($sidebar_cats as $scat) :
                ...
                <li class="flex items-center justify-between">
                  <a href="<?php echo esc_url(get_category_link($scat->term_id)); ?>" class="hover:text-primary transition-colors"><?php echo esc_html($scat->name); ?></a>
                  <span class="text-[10px] text-slate-400 dark:text-slate-600 bg-slate-100 dark:bg-slate-800/80 px-2 py-0.5 rounded-full"><?php echo intval($scat->count); ?></span>
                </li>
            <?php endforeach; ?>
          </ul>
        </div>
